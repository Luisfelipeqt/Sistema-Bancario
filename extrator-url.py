...
class ExtratorURL:
    ...
    def get_valor_parametro(self, parametro_busca):
        indice_parametro =  self.get_url_parametros().find(parametro_busca)# Encontra o moedaOrigem
        indice_valor = indice_parametro + len(parametro_busca) + 1 # Começa a partir "?" e vai até o moedaOrigem + 1 que é o =
        indice_e_comercial =  self.get_url_parametros().find("&", indice_valor) #Encontra primeiro o "&" a partir do indice_valor que é o moedaOrigem +, o .find quando não encontra VALOR ele retorna -1"""
